...
interface BulkCustomerUploadDialogProps {
  open: boolean;
...
const FIELD_META: Array<{ key: string; required: boolean; note: string }> = [
  { key: 'name', required: true, note: 'Customer full name' },
  { key: 'phone', required: true, note: 'Phone number with country code preferred' },
  { key: 'customer_type', required: false, note: 'individual/business (defaults to individual)' },
  { key: 'email', required: false, note: 'Optional email' },
  { key: 'company_name', required: false, note: 'Required for business customers' },
  { key: 'gst_number', required: false, note: 'Optional GST number' },
  { key: 'address', required: false, note: 'Address line' },
  { key: 'city', required: false, note: 'City' },
  { key: 'state', required: false, note: 'State' },
  { key: 'pincode', required: false, note: 'Postal code' },
...
const TEMPLATE_EXAMPLE: Record<string, string> = {
  name: 'Acme Retail LLP',
  phone: '+919876543210',
  customer_type: 'business',
  email: 'accounts@acmeretail.com',
  company_name: 'Acme Retail LLP',
...
function mapRowToCustomer(row: Record<string, unknown>): CreateCustomerData {
  const name = getValue(row, ['name', 'customer_name']);
  const phone = getValue(row, ['phone', 'phone_number', 'mobile']);
  const customerTypeRaw = getValue(row, ['customer_type', 'type']).toLowerCase();
  const customer_type: 'individual' | 'business' = customerTypeRaw === 'business' ? 'business' : 'individual';
  ...
  return {
    ...
    company_name: getValue(row, ['company_name']) || undefined,
...
export default function BulkCustomerUploadDialog({ open, onOpenChange, onSuccess }: BulkCustomerUploadDialogProps) {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      if (!rows.length) throw new Error('File is empty. Add at least one customer row.');
      ...
      for (let index = 0; index < rows.length; index += 1) {
        ...
        const rowName = getValue(row, ['name', 'customer_name']) || `Row ${rowNumber}`;
        try {
          const payload = mapRowToCustomer(row);
      ...
      if (successCount > 0) {
        toast({ title: 'Bulk import completed', description: `${successCount} customers imported, ${failedCount} failed.` });
        onSuccess?.();
      } else {
        toast({ title: 'Import failed', description: 'No customers were imported.', variant: 'destructive' });
      }
    } catch (error) {
      toast({
        title: 'Import failed',
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="max-w-3xl max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>Upload customers using CSV or Excel. Download sample format before import.</DialogDescription>
        </DialogHeader>
            ...
            <div className="rounded-lg border border-gray-200 p-4 space-y-3">
              ...
              {summary.success > 0 && (
                <div className="text-xs rounded-md border border-green-200 bg-green-50 p-2 text-green-700 flex items-center gap-2">
                  ...
                  Imported {summary.success} customer(s) successfully.
                </div>
              )}
            </div>
          )}
