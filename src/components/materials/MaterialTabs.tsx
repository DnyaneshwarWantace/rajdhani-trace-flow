...
export default function MaterialTabs({ activeTab, onTabChange, unreadCount, wasteCount, assignedTaskCount = 0 }: MaterialTabsProps) {
  return (
    <div className="mb-6 w-full">
      <div className="bg-gray-100 rounded-lg p-1 w-full">
        <nav className="flex gap-1 w-full" aria-label="Tabs">
          ...
          <button
            onClick={() => onTabChange('assigned-tasks')}
          ...
          >
            ...
            {assignedTaskCount > 0 && (
              <span className="inline-flex items-center justify-center px-2 py-0.5 text-xs font-semibold bg-indigo-100 text-indigo-700 rounded-full">
                {assignedTaskCount}
              </span>
            )}
          </button>
