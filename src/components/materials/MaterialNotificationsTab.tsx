...
import {
  Bell, CheckCircle, Clock, AlertTriangle, Info, ChevronDown, ChevronUp, X, Activity,
} from 'lucide-react';
...
export default function MaterialNotificationsTab(_props: MaterialNotificationsTabProps) {
  ...
  const loadAll = async () => {
    ...
    try {
      const [notifRes, logsRes] = await Promise.all([
        NotificationService.getNotifications({ module: 'materials', include_logs: 'false', limit: 500 }),
        NotificationService.getNotifications({ module: 'materials', include_logs: 'true', limit: 500 }),
      ]);

      const notifs = notifRes.data.filter(n => n.type !== 'activity_log');
      const logsOnly = logsRes.data.filter(n => n.type === 'activity_log');
      ...
      <div className="flex bg-gray-100 rounded-2xl p-1 gap-1">
        <button
          ...
          className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl text-xs font-bold transition-all ${
            viewMode === 'notifications' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-500'
          }`}
        >
        ...
        <button
          ...
          className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl text-xs font-bold transition-all ${
            viewMode === 'logs' ? 'bg-white text-purple-600 shadow-sm' : 'text-gray-500'
          }`}
        >
      ...
      {viewMode === 'notifications' && (
        <div className="flex items-center gap-2 overflow-x-auto pb-0.5 scrollbar-none">
          {(['all', 'unread', 'read'] as StatusFilter[]).map(s => (
            <button
              ...
              className={`shrink-0 px-3 py-1.5 rounded-full text-[11px] font-bold border transition-all ${
                statusFilter === s ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-gray-500 border-gray-200'
              }`}
            >
          ...
          {(['all', 'high', 'medium', 'low'] as PriorityFilter[]).map(p => (
            ...
            >
              {p !== 'all' && (
                <span className={`w-1.5 h-1.5 rounded-full ${priorityFilter === p ? 'bg-white/70' : PRIORITY_DOT[p]}`} />
              )}
            ...
            return (
              ...
              >
                ...
                <button
                  ...
                  onClick={() => toggle(n.id)}
                >
                  <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 ${getTypeIconBg(n.type)}`}>
                    {getTypeIcon(n.type)}
                  </div>
                  ...
                  <div className="border-t border-gray-50 bg-gray-50/70 px-4 py-3 space-y-2">
                    ...
                    {rd.materialName && (
                      <div className="flex justify-between text-xs">
                        ...
                        <span className="font-bold text-gray-800">{rd.materialName}</span>
                      </div>
                    )}
                    ...
                    {rd.shortage !== undefined && rd.shortage > 0 && (
                      <div className="flex justify-between text-xs">
                        ...
                        <span className="font-bold text-red-500">{rd.shortage} {rd.unit || ''}</span>
                      </div>
                    )}
                    {rd.batch_number && (
                      <div className="flex justify-between text-xs">
                        <span className="text-gray-400">Batch</span>
                        <span className="font-bold text-gray-800">{rd.batch_number}</span>
                      </div>
                    )}
                    {rd.created_by_user && (
                      <div className="flex justify-between text-xs">
          ...
          {viewMode === 'notifications' ? (
            <>
              ...
              <p className="text-xs text-gray-400">
                {activeFilters > 0 ? 'No notifications match your filters' : 'No pending material alerts'}
              </p>
              {activeFilters > 0 && (
                <button
                  onClick={() => { setPriorityFilter('all'); setStatusFilter('all'); }}
            ...
            <>
              ...
              <p className="text-xs text-gray-400">Material activity logs will appear here</p>
            </>
          )}
        </div>
      )}
