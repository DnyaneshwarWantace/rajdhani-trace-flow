                      ...
                      <div className="space-y-0.5">
                          {order.items.slice(0, 2).map((item, idx) => (
                            ...
                            <div className="text-gray-500">Qty: {Number(item.quantity).toFixed(2)} {item.count_unit || item.unit || 'units'}</div>
                              {(item.color || (item.productType === 'product' && item.pattern) || (item.length && item.width)) && (
                                <div className="flex flex-wrap items-center gap-1 mt-0.5">
                                  {item.length && item.width && (
                                    <span className="text-[10px] text-gray-500 tabular-nums">
                                      {item.length}
                                      {item.length_unit || ''}×{item.width}
                                      {item.width_unit || ''}
                                    </span>
                                  )}
                                  {item.color && colorCodeMap[item.color] && (
                                    <ColorSwatch colorCode={colorCodeMap[item.color]} className="w-3 h-3 rounded-sm shrink-0" />
                                  )}
                                  {item.color && !colorCodeMap[item.color] && (
                                    <span className="text-[10px] text-gray-500 truncate max-w-[56px]" title={item.color}>
                                      {item.color}
                                    </span>
                                  )}
                                  {item.productType === 'product' && item.pattern && patternImageMap[item.pattern] && (
                                    <img
                                      src={patternImageMap[item.pattern]}
                                      alt={item.pattern || 'Pattern'}
                                      className="w-3.5 h-3.5 rounded object-cover border border-gray-200 shrink-0"
                                    />
                                  )}
                                  {item.productType === 'product' && item.pattern && !patternImageMap[item.pattern] && (
                                    <span className="text-[10px] text-gray-500 truncate max-w-[72px]" title={item.pattern}>
                                      {item.pattern}
                                    </span>
                                  )}
                                </div>
