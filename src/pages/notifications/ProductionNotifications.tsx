...
export default function ProductionNotifications() {
  ...
  const loadData = async () => {
    try {
      ...
      // Filter for production/recipe-related notifications
      const productionNotifications = data.filter(n => 
        n.module === 'production' ||
      ...
      const regularNotifications = productionNotifications.filter(n => !n.related_data?.activity_log_id);
      const logs = productionNotifications.filter(n => n.related_data?.activity_log_id);
  ...
  const handleNotificationClick = (notification: Notification) => {
    if (notification.module === 'production' && notification.related_id) {
      navigate(`/production?highlight=${notification.related_id}`);
    } else if (notification.related_data?.action_category === 'RECIPE' && notification.related_id) {
      navigate(`/recipes?highlight=${notification.related_id}`);
    }
  };
    ...
    <Layout>
      <div>
        <SectionHeader
          ...
          description="View all production and recipe related notifications and logs"
          icon={<ChefHat className="w-8 h-8 text-teal-600" />}
        />
